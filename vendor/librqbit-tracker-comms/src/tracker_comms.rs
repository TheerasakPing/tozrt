...
impl TrackerComms {
    ...
    async fn task_single_tracker_monitor_http(&self, mut tracker_url: Url) -> anyhow::Result<()> {
        ...
        let base_query = tracker_url.query().map(str::to_owned);
        ...
        loop {
            ...
            let mut merged_query = base_query.clone().unwrap_or_default();
            if !merged_query.is_empty() {
                merged_query.push('&');
            }
            merged_query.push_str(&request_query);
            tracker_url.set_query(Some(&merged_query));
